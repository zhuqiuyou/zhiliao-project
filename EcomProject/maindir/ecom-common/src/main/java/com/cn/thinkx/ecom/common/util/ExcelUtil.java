...
public class ExcelUtil<T> {
	...
	private HSSFWorkbook workbook1 = null;
	private HSSFWorkbook workbook2 = null;
	 ...
	 */
	public void upLoadExcel(String title, String[] headers, Collection<T> dataset, OutputStream out){
		workbook1 = new HSSFWorkbook();  
		HSSFSheet sheet = workbook1.createSheet(title);  
		//设置表格默认列宽度
		...
		//生成另一个字体  
		HSSFFont font2 = workbook1.createFont();  
		...
		while (it.hasNext()) {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				try {
					...
					logger.error("## 导出表格得到类方法转换出现异常", e);
				}
			}
		}
		try {
			...
			logger.error("## 导出表格出现异常", e);
		}
	}

	public void upLoadExcel(String title, String titlerow, String startTime, String endTime, String[] titlehead, String[] titleheadnum,
			String[] head, Collection<T> dataset, Class<?> c, Object obj, HttpServletResponse resp){
		//创建excel表
		workbook2 = new HSSFWorkbook();  
		HSSFSheet sheet = workbook2.createSheet(title);  
		//设置默认行宽
		sheet.setDefaultColumnWidth((short) 20); 
		//表头样式（加粗，水平居中，垂直居中）
		HSSFCellStyle style = workbook2.createCellStyle();
		// 设置这些样式  
		...
		style.setBorderRight(HSSFCellStyle.BORDER_THIN);//上边框  
		style.setBorderTop(HSSFCellStyle.BORDER_THIN);//右边框  
		//设置字体  
		HSSFFont font = workbook2.createFont();
		font.setFontName("微软雅黑");
		...
		//生成另一个字体  
		HSSFFont font2 = workbook2.createFont();  
	...
	public static void upLoad(HSSFWorkbook workBook, String title, HttpServletResponse resp) throws IOException {
		OutputStream out = resp.getOutputStream();
		resp.reset();
		resp.setHeader("Content-disposition", "attachment; filename = "+ new String((title + ".xls").getBytes(), "iso-8859-1"));
