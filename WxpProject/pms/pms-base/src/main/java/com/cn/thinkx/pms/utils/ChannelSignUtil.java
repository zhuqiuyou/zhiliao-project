...
public class ChannelSignUtil {
	 ...
	 * @return
	 */
	public static String genSign(Object obj,String channelKey) {
	 ...
	 * @return
	 */
	public static String sign(SortedMap<String, String> items, String channelKey) {
		...
		if (StringUtil.isNullOrEmpty(channelKey)) {
			key = ReadPropertiesFile.getInstance().getProperty("COMMON_SIGN_KEY", null);
		} else {
			key = channelKey;
		}
	...
	public static boolean isSignExpired(long currentTime, long txnTimestamp) {
		int signatureTimeout = Integer.parseInt(ReadPropertiesFile.getInstance().getProperty("SIGNATURE_TIMEOUT", null));
