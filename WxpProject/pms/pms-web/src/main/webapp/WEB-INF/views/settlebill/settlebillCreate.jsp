...
<c:if test="${fn:contains(sessionInfo.resourceList, '/settlebillCreate/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/settlebillCreate/download')}">
	<script type="text/javascript">
		$.canDownload = true;
	</script>
</c:if>
...
<title>结算单生成</title>
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}/settlebill/dataGrid/00',
			striped : true,
			rownumbers : true,
			pagination : true,
			fitColumns:true,
				...
				formatter : function(value, row, index) {
					...
					if(row.isdefault!=0 ){
						...
						if ($.canEdit) {
							...
							str += $.formatString('<a href="javascript:void(0)" onclick="editFun(\'{0}\');" >调整</a>', row.settleId);
						}
					}
					return str;
				}
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 600,
			height : 350,
			href : '${ctx}/settlebill/editPage?settleId=' + id,
			buttons : [{
				text : '调整',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;
					var f = parent.$.modalDialog.handler.find('#settlebillEditForm');
					f.submit();
				}
			} ]
		});
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/settlebillCreate/updateStat')}">
			<a onclick="updateStat();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon_add'">结算单生成</a>
		</c:if>
	</div>
</body>
</html>