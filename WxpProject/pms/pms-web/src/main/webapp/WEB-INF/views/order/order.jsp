...
<c:if test="${fn:contains(sessionInfo.resourceList, '/order/download')}">
	<script type="text/javascript">
		$.canDown = true;
	</script>
</c:if>
...
<c:if test="${fn:contains(sessionInfo.resourceList, '/order/submit')}">
	<script type="text/javascript">
		$.canSubmit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/order/finish')}">
	<script type="text/javascript">
		$.canFinish = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/order/detailList')}">
	<script type="text/javascript">
		$.detailList = true;
	</script>
</c:if>
...
<title>制卡订单管理</title>
			...
			columns : [ [{
				...
				formatter : function(value, row, index) {
					...
					if(row.isdefault!=0){
						...
						if ($.canDown && row.orderStat=='13') {
							str += '&nbsp;&nbsp;|&nbsp;&nbsp;';
							str += $.formatString('<a href="javascript:void(0)" onclick="downLoadFun(\'{0}\');" >下载</a>', row.id);
						}
						...
						if ($.canFinish && row.orderStat=='14') {
							str += '&nbsp;&nbsp;|&nbsp;&nbsp;';
							str += $.formatString('<a href="javascript:void(0)" onclick="finishFun(\'{0}\');" >完成</a>', row.id);
						}
	...
	function finishFun(id){
		//window.location='${ctx}/order/finish?id='+id;
		//dataGrid.datagrid('reload');
		...
		parent.$.messager.confirm('询问', '您是否要完成当前订单？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/order/finish', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width :500,
			height : 250,
			href : '${ctx}/order/viewPage?type=100100&id=' + id
		});
	}
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width :500,
			height : 250,
			href : '${ctx}/order/editPage?type=100100&id=' + id,
			buttons : [{
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;
