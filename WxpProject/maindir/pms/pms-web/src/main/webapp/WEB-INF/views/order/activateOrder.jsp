...
<c:if test="${fn:contains(sessionInfo.resourceList, '/order/activateOrderEdit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/order/activateOrderDelete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/order/activateOrderSubmit')}">
	<script type="text/javascript">
		$.canSubmit= true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/order/activateOrderDetail')}">
	<script type="text/javascript">
		$.detailList= true;
	</script>
</c:if>
...
<title>激活订单管理</title>
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width :600,
			height : 600,
			href : '${ctx}/order/viewPage?type=300100&id=' + id
		});
	}
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width :600,
			height : 600,
			href : '${ctx}/order/editPage?type=300100&id=' + id,
			buttons : [{
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/order/activateOrderAdd')}">
