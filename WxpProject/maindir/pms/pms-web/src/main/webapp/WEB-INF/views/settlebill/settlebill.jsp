...
<c:if test="${fn:contains(sessionInfo.resourceList, '/settlebill/download')}">
...
<title>结算单查询</title>
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}/settlebill/dataGrid/all',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
		...
		<form id="searchForm">
			<table>
				<tr>
					...
					<th >结算单状态:</th>
					<td><select id="billStat" name="billStat"  class="easyui-combobox" data-options="required:true,width:140,height:29,editable:false,panelHeight:'auto'">
						<option selected value="">请选择</option>
						<c:forEach items="${billStatList}" var="obj" >
